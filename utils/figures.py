...
def income_comparison_slope_graph():
    ...
    fig.add_annotation(
        ...
        xref="paper",
        text=f"<b>{percent_diff:.2f}% difference</b>",
        showarrow=True,
        arrowhead=2,
...
def social_media_usage_comparison_line_chart():
    df_usage = pd.read_csv(
        "assets/Average hours per day spent on socializing and communicating by the U.S. population from 2009 to 2023.csv")
    ...
    df_world = pd.read_csv(
        "assets/Daily time spent on social networking by internet users worldwide from 2012 to 2024 - Sheet1.csv")
...
def jobs_vs_social_media_line_chart():
    df_jobs = pd.read_csv(
        "assets/Number of jobs created by start-up businesses that were less than one year old in the United States from 1994 to 2023 - Sheet1.csv")
    ...
    fig.update_layout(
        title='<b>New Jobs vs Social Media Usage</b>',
        xaxis_title="Year",
        yaxis_title="Value",
        title_x=0.5
    )
